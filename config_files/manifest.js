...
module.exports = (api, pluginOptions, packageJson) => async (content) => {
  ...
  if (isProduction) {
    ...
    if (manifest.content_security_policy.extension_pages.includes('connect-src \'self\'')) {
      manifest.content_security_policy.extension_pages = manifest.content_security_policy
        .extension_pages.replace('connect-src \'self\'', 'connect-src \'self\' ws://localhost:9090/');
    } else {
